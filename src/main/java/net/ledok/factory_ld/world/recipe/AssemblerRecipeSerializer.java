...
public class AssemblerRecipeSerializer implements RecipeSerializer<AssemblerRecipe> {
    ...
    private static final MapCodec<AssemblerRecipe> CODEC = RecordCodecBuilder.mapCodec(instance ->
        instance.group(
            ...
            Codec.INT.optionalFieldOf("craft_time", 120).forGetter(ConstructorRecipe::getCraftTime)
        ).apply(instance, AssemblerRecipeSerializer::fromCodec)
    );

    private static final StreamCodec<RegistryFriendlyByteBuf, AssemblerRecipe> STREAM_CODEC =
        StreamCodec.of(AssemblerRecipeSerializer::toNetwork, AssemblerRecipeSerializer::fromNetwork);
    ...
    private static AssemblerRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
        ...
        return new AssemblerRecipe(name, nameKey, category, researchGroup, inputs, outputs, craftTime);
    }

    private static void toNetwork(RegistryFriendlyByteBuf buf, AssemblerRecipe recipe) {
    ...
    private static AssemblerRecipe fromCodec(
        String recipeName,
        ...
        if (itemInputs.size() != 2 || itemOutputs.isEmpty()) {
            throw new IllegalStateException("Assembler recipes must define exactly 2 inputs and at least 1 output.");
        }
        return new AssemblerRecipe(recipeName, recipeNameKey, category, researchGroup, itemInputs, itemOutputs, craftTime);
    }
}
