...
public abstract class SingleVariantStorage<T extends TransferVariant<?>> extends SnapshotParticipant<ResourceAmount<T>> implements SingleSlotStorage<T> {
	...
	/**
	 * Read a {@link SingleVariantStorage} from a {@link class_11368}.
	 *
	 ...
	 * @param data the @{@link class_11368} instance to read from
	 * @param <T> the type of the item variant
	 */
	public static <T extends TransferVariant<?>> void readData(SingleVariantStorage<T> storage, Codec<T> codec, Supplier<T> fallback, class_11368 data) {
		storage.variant = data.method_71426("variant", codec).orElseGet(fallback);
		storage.amount = data.method_71425("amount", 0L);
	}

	/**
	 * Write a {@link SingleVariantStorage} to {@link class_11372}.
	 *
	 * @param storage the {@link SingleVariantStorage} to write from
	 * @param codec the item variant codec
	 * @param data the @{@link class_11372} instance to write from
	 * @param <T> the type of the item variant
	 */
	public static <T extends TransferVariant<?>> void writeData(SingleVariantStorage<T> storage, Codec<T> codec, class_11372 data) {
		data.method_71468("variant", codec, storage.variant);
		data.method_71466("amount", storage.amount);
	}
}
